...
class MuaPage extends StatefulWidget {
  const MuaPage({super.key});

  @override
  State<MuaPage> createState() => _MuaPageState();
}

class _MuaPageState extends State<MuaPage> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    // Ambil hanya package MUA
    final muaPackages = vendorProvider.muaPackages;
    
    // Kelompokkan berdasarkan kategori (contoh sederhana)
    final semuaMua = muaPackages;
    final terdekat = muaPackages.take(3).toList();
    final termurah = [...muaPackages]..sort((a, b) => a.price.compareTo(b.price));
    final yearbook = muaPackages.where((p) => p.name.contains('Yearbook')).toList();
                ...
                children: [
                  ...
                  if (semuaMua.isEmpty)
                    ...
                    if (terdekat.isNotEmpty) ...[
                      _buildSectionTitle("MUA Terdekat"),
                    ...
                    if (termurah.isNotEmpty) ...[
                      _buildSectionTitle("MUA Termurah"),
                      const SizedBox(height: 15),
                    ...
                    if (yearbook.isNotEmpty) ...[
                      _buildSectionTitle("MUA Yearbook"),
                      const SizedBox(height: 15),
                    ...
                    // Semua MUA
                    _buildSectionTitle("Semua MUA"),
                    const SizedBox(height: 15),
                    _buildGridList(semuaMua),
